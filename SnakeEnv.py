...
class SnakeEnv2(gym.Env):
    ...
    def step(self, action):
        self.time_steps += 1
        self.__direction = self.__new_dir(action)
        head_pos = self.get_geom_pos(self.body[0]) / self.__tile_size
        tail_pos = self.get_geom_pos(self.body[-1]) / self.__tile_size
        new_head_pos = (self.get_geom_pos(self.body[0]) + self.__direction * self.__tile_size) / self.__tile_size

        # quitting conditions
        if not self.__validate_pos(new_head_pos, self.body[1:-1]) \
                or self.time_steps >= 1000:
        ...
        if (self.get_geom_pos(self.body[0]) == self.get_geom_pos(self.__fruit)).all():
            ...
            self.body = [self.body[0]] + [new_body_block] + self.body[1:]
            self.set_geom_pos(self.__fruit, self.__new_fruit_pos() * self.__tile_size)
            # self.set_geom_pos(self.__fruit, (head_pos + np.array((1, -1))) * self.__tile_size)
        else:
            self.body = [self.body[0]] + self.body[-1:] + self.body[1:-1]
    ...
    def __new_fruit_pos(self):
        while True:
            ...
            if self.__validate_pos(new_fruit_pos, self.body):
                break
    ...
    def test(self):
        ...
        # test 2
        assert self.pos_in_body(np.array((5, 5)), self.body)
        assert self.pos_in_body(np.array((5, 4)), self.body)
        assert not self.pos_in_body(np.array((1, 2)), self.body)
        assert not self.pos_in_body(np.array((2, 1)), self.body)
        ...
        # test 4
        assert self.__validate_pos(np.array((0, 0)), self.body)
        assert self.__validate_pos(np.array((0, 1)), self.body)
        assert self.__validate_pos(np.array((1, 0)), self.body)
        assert not self.__validate_pos(np.array((0, -1)), self.body)
    ...
    def __tiles_to_obs(self):
        ...
        distance = -1
        while self.__validate_pos(cursor, self.body[1:-1]):
            distance += 1
            cursor += self.__direction
        return distance


    def pos_in_body(self, a, body):
        for x in body:
            coords = self.get_geom_pos(x) / self.__tile_size
            if (a == coords).all():
                return True
        return False
    ...
    def __validate_pos(self, pos, custom_body):
        if (pos < self.__grid).all() \
                and (pos >= np.array((0, 0))).all() \
                and not self.pos_in_body(pos, custom_body):
            return True
        return False
